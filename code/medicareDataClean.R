# Read Medicare Hospital data, clean it, and write it to a file
#=====================================================
dat <- readRDS("../data/ca_hospital_general_info.rds")
hospClean <- dat %>% mutate("TYPE_LIC" = NA, "CENS_TRACT" = NA) %>%
  dplyr::select("Hospital Name","address","lat","lon","County Name","TYPE_LIC", "CENS_TRACT", "City", "Hospital Type","Emergency Services", "Provider ID")
...
write.csv(tidyNearest3DF, file = "../data/GeoDistances_Medicare.csv", row.names = FALSE)
